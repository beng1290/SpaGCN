...
def calculate_p(adj, lim):
    """probability"""
    adj_exp = np.exp(-1 * (adj**2) / (2 * (lim**2)))
...
def find_l(p, adj, start=0.5, end=2, sep=0.01, tol=0.01):
    for lim in np.arange(start, end, sep):
        q = calculate_p(adj, lim)
        # print("L=", str(lim), "P=", str(round(q, 5)))
        if np.abs(p - q) <= tol:
            return lim
...
def find_neighbor_clusters(target_cluster, cell_id, x, y, pred, radius, ratio=1 / 2):
    ...
    for _, row in target_df.iterrows():
        ...
        y = row["y"]
        tmp_nbr = df[((df["x"] - x) ** 2 + (df["y"] - y) ** 2) <= (radius**2)]
        # tmp_nbr=df[(df["x"]<x+radius) & (df["x"]>x-radius) &
        # (df["y"]<y+radius) & (df["y"]>y-radius)]
        num_nbr.append(tmp_nbr.shape[0])
...
def refine(sample_id, pred, dis, shape="hexagon"):
    ...
    if shape == "hexagon":
        ...
        e1 = "Shape not recongized, shape='hexagon' for Visium data"
        e2 = "'square' for ST data."
